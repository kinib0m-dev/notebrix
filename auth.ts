...
import {
  ...
  handleWelcomeEmail,
} from "./lib/auth/helpers/welcome";

export type UserRole = "user" | "admin";

export const { auth, handlers, signIn, signOut } = NextAuth({
  events: {
    // This is to populate the emailVerified field when signing in from an oauth
    ...
    // Handle OAuth user first login for welcome email
    async signIn({ user, account }) {
      // Only handle OAuth sign-ins and check for welcome email
      if (
        ...
        const welcomeEmailCheck = await checkWelcomeEmailOnLogin(user.id);
  ...
  callbacks: {
    async signIn({ user, account }) {
      ...
      if (account?.provider !== "credentials") {
        // For OAuth, update the signup method if it's a new user
        if (user.id) {
